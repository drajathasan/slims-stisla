...
defined('SB') or die('Direct access not allowed!');
...
$custom_dir = new simbio_directory(STISLA_TEMPLATE);
$dir_tree = array_merge($dir_tree, $custom_dir->getDirectoryTree(1));
...
foreach ($dir_tree as $dir) {
  $is_custom = '';
  if (!is_dir(SB . 'admin/admin_template/' . $dir)) $is_custom = '&custom_temolate=true';
  $_btn = '<a href="' . MWB . 'system/theme.php?customize=admin&theme=' . $dir . $is_custom . '" data-value="' . $dir . '" class="btn btn-default notAJAX setAdminTheme">' . __('Activate') . '</a>';
  if ($dir == $sysconf['admin_template']['theme']) {
    $_btn = '<a href="' . MWB . 'system/theme.php?customize=admin&theme=' . $dir . '" data-value="' . $dir . $is_custom . '" title="' . __('Admin Theme Configuration') . '" class="btn btn-success notAJAX customeAdminTheme openPopUp">' . __('Customize') . '</a>';
  }
  $output = '<div class="col-3">';
  $output .= '<div class="card border-0 mb-4">';
  $output .= '<div class="card-body">';
  $output .= '<div class="mb-2 font-weight-bold">' . ucwords($dir) . '</div>';
  $originalPath = SB.'admin/admin_template/'.$dir.'/preview';
  $customPath = STISLA_TEMPLATE . '/preview';
  
  if (file_exists($fullPath = $originalPath .  '.jpg') || file_exists($fullPath = $originalPath .  '.png')) {
    $output .= '<img class="card-img-top rounded" src="'.SWB.'admin/admin_template/'. $dir . '/'.basename($preview).'" height="150" />';
  } else {
    $output .= '<img class="card-img-top rounded" src="'.STISLA_URL . '/'.$preview.'" height="150" />';
  }
  $output .= '</div>';
  $output .= '<div class="card-footer border-0">' . $_btn . '</div>';
  $output .= '</div>';
